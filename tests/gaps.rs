...
fn ignore_whitespace_resolves_a_reindent_adjacent_fix() {
    ...
    assert!(
        matches!(ops::fix(&t.repo, &c1.to_string(), false), Err(Error::Conflict { .. })),
...
fn fix_rejects_non_ancestor_target() {
    ...
    t.stage(&[("a.txt", "3\n")]);
    assert!(matches!(
        ops::fix(&t.repo, &sib.to_string(), false),
...
fn move_target_is_intro_errors() {
    ...
    assert!(matches!(
        ops::mv(&t.repo, "feature.txt", &c2.to_string(), false),
        Err(Error::Empty(_))
    ));
}
...
fn move_handles_nested_paths() {
    ...
    let out = ops::mv(&t.repo, "src/lib.rs", &c3.to_string(), false).unwrap();
    let c2p = t.nth_parent(out.new_tip, 1);
